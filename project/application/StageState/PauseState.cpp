...
void PauseState::Update(StageScene* scene) {
	...
	if (gPoseAnimEnabled) {
		...
		if (gPoseAlpha > 1.0f) gPoseAlpha = 1.0f;
	} else {
		gPoseAlpha = 1.0f;
	}
	...
	if (resumeText_) resumeText_->SetPosition({gItemX, gResumeY});
	if (restartText_) restartText_->SetPosition({gItemX, gRestartY});
	if (titleText_) titleText_->SetPosition({gItemX, gTitleY});
	if (cursor_) cursor_->SetSize({gCursorSize, gCursorSize});
	UpdateCursor();
...
void PauseState::UpdateCursor() {
	if (!cursor_) return;
	float y = gResumeY;
	if (selected_ == 1) y = gRestartY;
	else if (selected_ == 2) y = gTitleY;
	// X/Y方向のオフセットを適用
