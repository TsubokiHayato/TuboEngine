...
void Enemy::Initialize() {
    ...
    if (!hitEmitter_) {
        ...
        p.colorStart = {1.0f,0.6f,0.2f,0.9f}; p.colorEnd = {1.0f,1.0f,0.4f,0.0f};
        p.center = position;
    ...
    if (!deathEmitter_) {
        ...
        p.scaleStart = {0.6f,0.6f,0.6f}; p.scaleEnd = {1.4f,1.4f,1.4f};
        p.colorStart = {1.0f,0.3f,0.2f,0.9f}; p.colorEnd = {1.0f,0.8f,0.1f,0.0f};
        p.center = position;
        deathEmitter_ = ParticleManager::GetInstance()->CreateEmitter<RingEmitter>(p);
    }
}

static float NormalizeAngle(float angle) { while (angle > kPI) angle -= 2.0f * kPI; while (angle < -kPI) angle += 2.0f * kPI; return angle; }
...
struct AStarCell { float g = std::numeric_limits<float>::infinity(); float f = std::numeric_limits<float>::infinity(); int parent = -1; bool closed = false; bool opened = false; };
static inline int Index1D(int x, int y, int w) { return y * w + x; }
static bool IsTileWalkable(MapChipField* field, uint32_t x, uint32_t y) { if (!field) return false; if (x >= field->GetNumBlockHorizontal() || y >= field->GetNumBlockVirtical()) return false; Vector3 center = field->GetMapChipPositionByIndex(x, y); return !field->IsBlocked(center); }
static int FindNearestWalkableIndex(MapChipField* field, int gx, int gy) {
    const int W = (int)field->GetNumBlockHorizontal(); const int H = (int)field->GetNumBlockVirtical(); if (gx < 0 || gy < 0 || gx >= W || gy >= H) return -1; if (IsTileWalkable(field, (uint32_t)gx, (uint32_t)gy)) return Index1D(gx, gy, W);
    const int maxRadius = 6; for (int r = 1; r <= maxRadius; ++r) { for (int dy = -r; dy <= r; ++dy) { for (int dx = -r; dx <= r; ++dx) { int nx = gx + dx; int ny = gy + dy; if (nx < 0 || ny < 0 || nx >= W || ny >= H) continue; if (std::abs(dx) + std::abs(dy) != r) continue; if (IsTileWalkable(field, (uint32_t)nx, (uint32_t)ny)) { return Index1D(nx, ny, W); } } } } return -1; }

bool Enemy::BuildPathTo(const Vector3& worldGoal) { currentPath_.clear(); pathCursor_ = 0; lastPathGoalIndex_ = -1;
...
void Enemy::Update() {
    ...
    if (!isAllive) {
        ...
        // 死亡後も deathEmitter_ の center を追随させたいなら位置更新
    ...
    if (player_) {
        if (canSeePlayer) {
            if (distanceToPlayer > moveStartDistance_) state_ = State::Idle; else if (distanceToPlayer > shootDistance_) state_ = State::Chase; else state_ = State::Attack;
        } else if (lastSeenTimer > 0.0f) state_ = State::Alert; else if (state_ == State::Alert) state_ = State::LookAround; else if (state_ != State::LookAround) state_ = State::Idle;
    }
    ...
    // クールダウンタイマー更新
    if (wantShoot_) bulletTimer_ += dt; else bulletTimer_ = std::min(bulletTimer_, EnemyNormalBullet::s_fireInterval);
    auto TryFire = [this]() { if (!wantShoot_) return; if (bullet && bullet->GetIsAlive()) return; if (bullet && !bullet->GetIsAlive()) bullet.reset(); if (bulletTimer_ < EnemyNormalBullet::s_fireInterval) return; bulletTimer_ = 0.0f; bullet = std::make_unique<EnemyNormalBullet>(); bullet->Initialize(position); bullet->SetEnemyPosition(position); bullet->SetEnemyRotation(rotation); bullet->SetPlayer(player_); bullet->SetCamera(camera_); bullet->SetMapChipField(mapChipField); };

    // 状態ごとの行動
    switch (state_) {
    case State::Idle: {
        switch (idleBackPhase_) {
        case IdleBackPhase::None: idleLookAroundTimer -= dt; if (idleLookAroundTimer <= 0.0f) { idleBackStartAngle = rotation.z; idleBackTargetAngle = NormalizeAngle(idleBackStartAngle + kPI); idleBackHoldTimer = idleBackHoldSec; idleBackPhase_ = IdleBackPhase::ToBack; } break;
        case IdleBackPhase::ToBack: { float diff = NormalizeAngle(idleBackTargetAngle - rotation.z); float turn = std::clamp(diff, -idleBackTurnSpeed, idleBackTurnSpeed); rotation.z = NormalizeAngle(rotation.z + turn); if (std::fabs(diff) < 0.01f) { rotation.z = idleBackTargetAngle; idleBackPhase_ = IdleBackPhase::Hold; } } break;
        case IdleBackPhase::Hold: idleBackHoldTimer -= dt; if (idleBackHoldTimer <= 0.0f) idleBackPhase_ = IdleBackPhase::Return; break;
        case IdleBackPhase::Return: { float diff = NormalizeAngle(idleBackStartAngle - rotation.z); float turn = std::clamp(diff, -idleBackTurnSpeed, idleBackTurnSpeed); rotation.z = NormalizeAngle(rotation.z + turn); if (std::fabs(diff) < 0.01f) { rotation.z = idleBackStartAngle; idleBackPhase_ = IdleBackPhase::None; idleLookAroundTimer = idleLookAroundIntervalSec; } } break; }
        break; }
    case State::Alert: { idleBackPhase_ = IdleBackPhase::None; break; }
    case State::LookAround: { idleBackPhase_ = IdleBackPhase::None; if (!lookAroundInitialized) { lookAroundBaseAngle = rotation.z; lookAroundTargetAngle = lookAroundBaseAngle + lookAroundAngleWidth; lookAroundDirection = 1; lookAroundCount = 0; lookAroundInitialized = true; } float diff = NormalizeAngle(lookAroundTargetAngle - rotation.z); float turn = std::clamp(diff, -lookAroundSpeed, lookAroundSpeed); rotation.z = NormalizeAngle(rotation.z + turn); if (std::fabs(diff) < 0.02f) { lookAroundDirection *= -1; lookAroundTargetAngle = lookAroundBaseAngle + lookAroundDirection * lookAroundAngleWidth; lookAroundCount++; if (lookAroundCount >= lookAroundMaxCount) { lookAroundInitialized = false; state_ = State::Patrol; } } break; }
    case State::Patrol: { idleBackPhase_ = IdleBackPhase::None; state_ = State::Idle; break; }
    case State::Chase: { idleBackPhase_ = IdleBackPhase::None; if (player_) { Vector3 dir = player_->GetPosition() - position; dir.z = 0.0f; float len = std::sqrt(dir.x*dir.x + dir.y*dir.y); if (len > 0.1f) { dir.x/=len; dir.y/=len; Vector3 desiredMove{dir.x*moveSpeed_, dir.y*moveSpeed_, 0.0f}; MoveWithCollision(position, desiredMove, mapChipField); } } TryFire(); break; }
    case State::Attack: { idleBackPhase_ = IdleBackPhase::None; TryFire(); break; }
    }

    if (bullet && bullet->GetIsAlive()) bullet->Update();
...
void Enemy::Draw() { if (isAllive == false) { /* 死亡後は通常モデル非表示 */ return; } if (object3d) object3d->Draw(); if (bullet) bullet->Draw(); DrawViewCone(); DrawLastSeenMark(); DrawStateIcon(); }
...
void Enemy::DrawStateIcon() {
    ...
    Vector4 col;
    switch (state_) {
    case State::Idle: col = {0.4f,0.4f,0.4f,0.8f}; break;
    case State::Alert: col = {1.0f,0.8f,0.2f,0.9f}; break;
    case State::LookAround: col = {0.2f,0.7f,1.0f,0.9f}; break;
    case State::Patrol: col = {0.3f,0.9f,0.3f,0.9f}; break;
    case State::Chase: col = {1.0f,0.2f,0.2f,0.9f}; break;
    case State::Attack: col = {1.0f,0.0f,1.0f,0.9f}; break;
    }
    float s = 0.4f;
    // 菱形
